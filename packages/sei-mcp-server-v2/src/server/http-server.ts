...
startServer().then(s => {
  ...
  app.get("/metrics", (req: Request, res: Response) => {
    ...
    const lines = [
      ...
      `mcp_active_connections ${connections.size}`,
      '# HELP mcp_process_uptime_seconds Process uptime in seconds',
      '# TYPE mcp_process_uptime_seconds gauge',
      `mcp_process_uptime_seconds ${process.uptime().toFixed(0)}`
    ];
  ...
  // Simple tool chaining endpoint for MCP v2-style workflows
  // @ts-ignore
  app.post('/mcp/v2/chain', async (req: Request, res: Response) => {
    try {
      ...
      for (const step of tools) {
        const handler = getToolHandler(server as any, step?.name);
        if (!handler || typeof handler.callback !== 'function') {
          results.push({ name: step?.name, error: 'Tool not found' });
          continue;
        }
        // eslint-disable-next-line no-await-in-loop
        const out = await handler.callback(step?.arguments || {});
        results.push({ name: step?.name, result: out });
      }
      return res.json({ session: sessionId || null, chain: chain || null, results });
    } catch (e) {
