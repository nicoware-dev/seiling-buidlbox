...
export async function resolvePgliteDir(
  ...
  // DEBUG: Add logging to track resolution process
  console.log('[DEBUG] resolvePgliteDir - dir parameter:', dir);
  console.log('[DEBUG] resolvePgliteDir - fallbackDir parameter:', fallbackDir);
  console.log('[DEBUG] resolvePgliteDir - targetProjectDir parameter:', targetProjectDir);
  console.log(
    '[DEBUG] resolvePgliteDir - process.env.PGLITE_DATA_DIR:',
  ...
  console.log('[DEBUG] resolvePgliteDir - projectRoot:', projectRoot);
  ...
  if (!targetProjectDir && pathsInfo.envFilePath && existsSync(pathsInfo.envFilePath)) {
    console.log('[DEBUG] resolvePgliteDir - Loading .env file from:', pathsInfo.envFilePath);
    dotenv.config({ path: pathsInfo.envFilePath });
    console.log(
      '[DEBUG] resolvePgliteDir - PGLITE_DATA_DIR after dotenv:',
      process.env.PGLITE_DATA_DIR
    );
  } else {
    console.log(
      '[DEBUG] resolvePgliteDir - Skipping .env loading (targetProjectDir provided or no .env found)'
    );
  }
  ...
  console.log('[DEBUG] resolvePgliteDir - defaultBaseDir:', defaultBaseDir);

  const base = dir ?? process.env.PGLITE_DATA_DIR ?? fallbackDir ?? defaultBaseDir;

  console.log('[DEBUG] resolvePgliteDir - resolved base:', base);
  ...
  if (resolved === legacyPath) {
    ...
    console.log('[DEBUG] resolvePgliteDir - migrated legacy path to:', newPath);
    return newPath;
  }

  console.log('[DEBUG] resolvePgliteDir - final resolved path:', resolved);
  return resolved;
}
