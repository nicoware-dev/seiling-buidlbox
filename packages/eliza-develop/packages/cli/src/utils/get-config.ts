...
export async function setupPgLite(
  ...
  console.log('[DEBUG] setupPgLite - dbDir parameter:', dbDir);
  console.log('[DEBUG] setupPgLite - envPath parameter:', envPath);
  console.log('[DEBUG] setupPgLite - targetProjectDir parameter:', targetProjectDir);

  const dirs = await ensureElizaDir(targetProjectDir);
  const { elizaDbDir, envFilePath } = dirs;

  console.log('[DEBUG] setupPgLite - elizaDbDir from ensureElizaDir:', elizaDbDir);
  console.log('[DEBUG] setupPgLite - envFilePath from ensureElizaDir:', envFilePath);

  // Use provided parameters or defaults from dirs
  const targetDbDir = dbDir || elizaDbDir;
  const targetEnvPath = envPath || envFilePath;

  console.log('[DEBUG] setupPgLite - targetDbDir:', targetDbDir);
  console.log('[DEBUG] setupPgLite - targetEnvPath:', targetEnvPath);

  try {
    // Ensure the PGLite database directory exists
    console.log('[DEBUG] setupPgLite - Creating directory:', targetDbDir);
    ...
    console.log('[DEBUG] setupPgLite - Error details:', error);
...
export async function configureDatabaseSettings(reconfigure = false): Promise<string | null> {
  // DEBUG: Add logging to track environment variable state
  console.log(
    '[DEBUG] configureDatabaseSettings - PGLITE_DATA_DIR at start:',
    process.env.PGLITE_DATA_DIR
  );
  console.log('[DEBUG] configureDatabaseSettings - Current working directory:', process.cwd());

  // Set up directories and env file
  const { elizaDbDir, envFilePath } = await ensureElizaDir();
  console.log('[DEBUG] configureDatabaseSettings - elizaDbDir from ensureElizaDir:', elizaDbDir);
  console.log('[DEBUG] configureDatabaseSettings - envFilePath from ensureElizaDir:', envFilePath);

  await setupEnvFile(envFilePath);
  await loadEnvironment(path.dirname(envFilePath));

  // DEBUG: Check environment after loading
  console.log(
    '[DEBUG] configureDatabaseSettings - PGLITE_DATA_DIR after loadEnvironment:',
    process.env.PGLITE_DATA_DIR
  );
  ...
  console.log(
    '[DEBUG] configureDatabaseSettings - pgliteDataDir from resolvePgliteDir:',
    pgliteDataDir
  );
