...
// DEBUG: Add early environment variable loading and logging
import dotenv from 'dotenv';
import path from 'node:path';
import { existsSync } from 'node:fs';

// Try to load .env file early - look in the monorepo root, not the CLI package directory
const cliDir = process.cwd();
const monorepoRoot = path.resolve(cliDir, '..', '..'); // Go up two levels from packages/cli to get to the root
const envPath = path.join(monorepoRoot, '.env');

console.log('[DEBUG] CLI startup - Current working directory:', process.cwd());
console.log('[DEBUG] CLI startup - Monorepo root:', monorepoRoot);
console.log('[DEBUG] CLI startup - Looking for .env file at:', envPath);
console.log('[DEBUG] CLI startup - .env file exists:', existsSync(envPath));

if (existsSync(envPath)) {
  console.log('[DEBUG] CLI startup - Loading .env file...');
  dotenv.config({ path: envPath });
  console.log('[DEBUG] CLI startup - PGLITE_DATA_DIR after dotenv:', process.env.PGLITE_DATA_DIR);
} else {
  console.log('[DEBUG] CLI startup - No .env file found at startup');
}
