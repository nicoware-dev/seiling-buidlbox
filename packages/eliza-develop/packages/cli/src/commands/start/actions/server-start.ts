...
export async function startAgents(options: ServerStartOptions): Promise<void> {
  ...
  // Default fallback to Eliza character
    ...
    // Ensure environment variables are loaded before creating the character
    logger.info('Loading default Eliza character with environment-based plugin selection...');
    logger.debug(`Environment check - OPENAI_API_KEY: ${process.env.OPENAI_API_KEY ? 'SET' : 'NOT SET'}`);
    logger.debug(`Environment check - ANTHROPIC_API_KEY: ${process.env.ANTHROPIC_API_KEY ? 'SET' : 'NOT SET'}`);
    logger.debug(`Environment check - GOOGLE_GENERATIVE_AI_API_KEY: ${process.env.GOOGLE_GENERATIVE_AI_API_KEY ? 'SET' : 'NOT SET'}`);
    
    const elizaCharacter = getElizaCharacter();
    logger.info(`Eliza character plugins: ${elizaCharacter.plugins?.join(', ')}`);
